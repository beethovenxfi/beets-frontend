...
import { CheckIcon } from '~/components/steps/components/Check';
import { CloseIcon } from '~/components/steps/components/Close';
...
export const StepIcon = forwardRef<StepIconProps, 'div'>((props, ref) => {
    ...
    return React.useMemo(() => {
        ...
        if (isCurrentStep) {
            if (isError) return <AnimatedCloseIcon key="icon" color="white" {...animationConfig} style={icon} />;
            if (isLoading) return <Spinner width={icon.width as string} height={icon.height as string} />;
        }
        if (Icon)
            return (
        ...
        return (
            <AnimatedSpan ref={ref} key="label" style={iconLabel} __css={labelStyles} {...animationConfig}>
                {(index || 0) + 1}
            </AnimatedSpan>
        );
