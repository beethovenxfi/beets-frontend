import { GqlPoolMetaStable } from '~/apollo/generated/graphql-codegen-generated';
 ...
 */
export class PoolMetaStableService implements PoolService {
    private baseService: PoolBaseService;

    constructor(
        private pool: GqlPoolMetaStable,
    ...
    public updatePool(pool: GqlPoolMetaStable) {
    ...
    public bptForTokensZeroPriceImpact(tokenAmounts: TokenAmountHumanReadable[]): OldBigNumber {
        ...
        const balances = this.pool.tokens.map((token, index) =>
            parseUnits(token.balance, token.decimals)
                //apply the price rate to the balances
                .mul(this.baseService.priceRatesScaled[index].toString())
                .div(WeiPerEther),
        );
